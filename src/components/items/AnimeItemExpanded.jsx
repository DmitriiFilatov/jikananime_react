...
function AnimeItemExpanded({ entry }) {
  const styleIcon = {
    fontSize: 20,
    color: "primary.main",
  };
  ...
  const airedFrom = new Date(entry.aired.from).toLocaleDateString(
    "en-US",
    langOptions
  );
  const airedTo = new Date(entry.aired.to).toLocaleDateString(
    "en-US",
    langOptions
  );
...
export default AnimeItemExpanded;
